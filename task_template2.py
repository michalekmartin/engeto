...
login = dict(bob="123", ann="pass123", mike="password123", liz="pass123")
...
if not login[username] == password:
    print("You put wrong username or password")
    quit()
...
print(40 * "-")
lst2 = TEXTS[number].split()
frequencies = dict()
while lst2:
    word = lst2.pop().strip(" ,.")
    if len(word) not in frequencies:
        frequencies.update({len(word): 1})
    else:
        frequencies[len(word)] = frequencies[len(word)] + 1

keys = sorted(list(frequencies))
while keys:
    key = keys.pop(0)
    print(key, "*" * frequencies[key], frequencies[key])
print(40 * "-")
